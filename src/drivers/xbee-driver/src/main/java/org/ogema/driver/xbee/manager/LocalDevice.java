...
public class LocalDevice {
	...
	public void setCyclicSleepPeriod(short cyclicSleepPeriod) {
		...
		if (logger.isDebugEnabled())
			logger.debug("***CyclicSlepp: " + Integer.toHexString(cyclicSleepPeriod & 0xffff));
	}
	...
	public void sendMessage(byte[] message) {
		if (Configuration.DEBUG && logger.isTraceEnabled())
			logger.trace("Start sending message: " + Constants.bytesToHex(message));
	...
	public void sendFrame(byte[] frame) {
		if (Configuration.DEBUG && logger.isTraceEnabled())
			logger.trace("Sending frame: " + Constants.bytesToHex(frame));
		...
		if (localEndpoint == 0x01 && clusterId == 0x000A) {
			...
			if ((Short.reverseBytes(payloadBuffer.getShort()) & 0xffff) == 0x0000) { // UTCTime read request
				...
				if (Configuration.DEBUG && logger.isDebugEnabled()) {
					logger.debug("*****UTC Time: " + secondsSinceEpoch);
