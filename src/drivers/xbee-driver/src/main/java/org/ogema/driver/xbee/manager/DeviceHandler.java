				...
				case UNINITIALIZED: // The endpoint is missing some information
					if (remoteDevice.getAddress16Bit() == 0) {
						...
						if (logger.isDebugEnabled())
							logger.debug("Sending network address request to: {}", Long.toHexString(remoteDevice.getAddress64Bit()));
					...
					if (remoteDevice.getEndpoints().isEmpty()) { // The
						...
						if (logger.isDebugEnabled()) {
							logger.debug("Sending active endpoints request to: "
								 + Long.toHexString(remoteDevice.getAddress64Bit()) + "\n" 
