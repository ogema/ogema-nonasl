...
public class InputHandler {
	...
	private void handleExplicitRxIndicator() {
		if (Configuration.DEBUG)
			logger.trace("Handle RX Indicator");
		sourceAddress64Bit = byteBuffer.getLong();
		sourceAddress16Bit = byteBuffer.getShort();
		...
		if (0 == profileId) { // ZDO Commands
			...
			if (Configuration.DEBUG)
				logger.trace("############################ Message from XBee device received!!!");
	...
	private void handleMgmtLqiResponse() {
		...
		if (debug) {
			logger.trace("MagmtLqiResponse!");
			logger.trace("startIndex: " + startIndex);
			logger.trace("count: " + neighborTableListCount);
			logger.trace("entries: " + neighborTableEntries);
		}
		for (int i = 0; i < (neighborTableListCount & 0xff); ++i) {
			if (debug)
				logger.trace("###neighbou table entry:");
			long extendedPanId = Long.reverseBytes(byteBuffer.getLong());
			if (debug)
				logger.trace("### ExtendedPanId: " + Long.toHexString(extendedPanId));
			long address64Bit = Long.reverseBytes(byteBuffer.getLong());
			if (debug)
				logger.trace("### 64bit Address: " + Long.toHexString(address64Bit));
			short address16Bit = Short.reverseBytes(byteBuffer.getShort());
			if (debug)
				logger.trace("### 16bit Address: " + Integer.toHexString(address16Bit & 0xffff));
	...
	private void handleMatchDescriptorRequest() {
		...
		if (logger.isDebugEnabled()) {
			logger.debug("Output cluster Count: " + outputClusters.length + "\n Input cluster Count: "
	...
	private void handleUserDescriptorResponse() {
		logger.debug("User Descriptor response!");
		byteBuffer.get(); // Omit frame control byte
		if (Constants.STATUS_SUCCESS != byteBuffer.get())
			return; // No success
		nwkAddrOfInterest = Short.reverseBytes(byteBuffer.getShort()); // Network
		...
		if (logger.isDebugEnabled())
			logger.debug("New user description: " + String.valueOf(userDescription));
	...
	private void handleSimpleDescriptorResponse() {
		...
		if (logger.isTraceEnabled()) {
			logger.trace("Simple descriptor response!");
			logger.trace("nwkAddrOfInterest: " + Integer.toHexString(nwkAddrOfInterest & 0xffff));
			logger.trace("EndpointId: " + Constants.bytesToHex(simpleDescriptor.getEndpointId()));
		}
