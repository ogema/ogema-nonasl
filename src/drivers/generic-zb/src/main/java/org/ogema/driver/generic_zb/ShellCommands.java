...
public class ShellCommands implements HLDriverInterface {
	...
	public void deviceScan(@Descriptor("The interface ID/Port name.") String interfaceId) throws Throwable {
		final ApplicationManager appMan = driver.appManager;
		if (appMan == null)  // do it only if the HLD app is running
			return;
		try {
			appMan.getChannelAccess().discoverDevices("xbee-driver", interfaceId, null, driver);
		} catch (Throwable e) {
			throw e;
		}
	}
