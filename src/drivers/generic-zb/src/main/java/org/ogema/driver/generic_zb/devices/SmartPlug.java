...
public class SmartPlug extends Generic_ZbDevice implements ResourceValueListener<BooleanResource> {
	...
	public ChannelConfiguration addChannel(Generic_ZbConfig config) {
		...
		case "COMMAND":
			try {
				...
				Generic_ZbDriver.logger.warn("Failed to add channel", e);
			}
			break;
		case "ATTRIBUTE":
			try {
				...
				driver.channelMap.put(config.resourceName, channelConfig);
			} catch (ChannelAccessException | NullPointerException e) {
				Generic_ZbDriver.logger.warn("Failed to add channel", e);
			}
			break;
		default:
	...
	public void resourceChanged(BooleanResource resource) {
		try {
			...
			Generic_ZbDriver.logger.warn("Failed to update channel value for resource {}", resource, e);
		}
	}
	...
	private void updateOnOffState() {
		try {
			...
			Generic_ZbDriver.logger.warn("Failed to update channel value for resource {}", isOn, e);
		}
	}
